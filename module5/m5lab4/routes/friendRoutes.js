...
router.get("/filter", (req, res) => {
  ...
  const { gender: filterGender, letter: filterLetter } = req.query;
  let matchingFriends = [...friends];

  // Filter by gender if the query parameter is present
  if (filterGender) {
  ...
  // Filter by letter if the query parameter is present
  if (filterLetter) {
    matchingFriends = matchingFriends.filter(
  ...
  if (matchingFriends.length > 0) {
    ...
    // Return an error response when no matches are found
...
router.get("/:id", (req, res) => {
  ...
  const friendId = req.params.id; // 'id' here will be a value matching anything after the / in the request path
